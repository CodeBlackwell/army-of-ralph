"""Ralph core — the autonomous coding agent runner (solo + army modes)."""
...
def detect_mode(target_dir: Path) -> bool:
    """Return True if the target dir is an army PRD (has an agents/ directory)."""
    return (target_dir / "agents").is_dir()
...
2. Read {self.progress_file} - check the Learnings section first for patterns from previous iterations.
...
After completing your task, just end your response. Ralph tracks progress automatically."""
    ...
    def _init_timing_log(self) -> None:
        ...
        with open(self.timing_log, "w", encoding="utf-8") as f:
            ...
            f.write(f"Initial Progress: {self.initial_progress.completed}/{self.initial_progress.total} tasks\n")
            f.write("---\n")
    ...
    def _log_iteration(self, iteration: int, iter_duration: float, total_elapsed: float) -> None:
        ...
        with open(self.timing_log, "a", encoding="utf-8") as f:
            f.write(f"Iteration {iteration}: {format_duration(iter_duration)} | Total: {format_duration(total_elapsed)}\n")

    def _log_summary(self, status: str, iterations: int, total_time: float, tasks_done: int, final_progress: TaskProgress) -> None:
        ...
        with open(self.timing_log, "a", encoding="utf-8") as f:
            ...
            f.write(f"Average per Iteration: {format_duration(total_time / iterations) if iterations > 0 else '00:00:00'}\n")
    ...
    def _print_summary(self, title: str, iterations: int, total_time: float, tasks_done: int, progress: TaskProgress) -> None:
        ...
        print(f"  Avg/Iter:      {format_duration(total_time / iterations) if iterations > 0 else '00:00:00'}")
    ...
    def _append_gate_failure(self, agent_name: str, wave_num: int, attempt: int, error: str) -> None:
    ...
    def _cleanup_agents(self, active: dict) -> None:
        ...
        for agent, (proc, fh) in active.items():
            ...
            while active:
                ...
                for agent, (proc, fh) in list(active.items()):
                    if self._check_agent_complete(agent):
                        ...
                        print(f"  ✗ {agent}-agent exited (code {proc.returncode}) without completion signal")
                        done.append(agent)
                for agent in done:
                    if agent in active:
    ...
    def run_army(self) -> int:
        ...
        for wave_num in sorted(waves.keys()):
            ...
            print(f"\n  Wave {wave_num}: {format_duration(wave_dur)} | Total: {format_duration(total_elapsed)}")
            with open(self.timing_log, "a", encoding="utf-8") as f:
        ...
        title = "Army complete!" if final.remaining == 0 else f"Army done — {final.remaining} tasks remaining"
    ...
    def run(self) -> int:
        ...
        for i in pbar:
            ...
            # Show timing
            print()
            print(f"⏱  Iter {i}: {format_duration(iter_duration)} | Total: {format_duration(total_elapsed)} | Avg: {format_duration(total_elapsed / i)}/iter")
        ...
        self._log_summary("MAX ITERATIONS", self.max_iterations, final_elapsed, tasks_done, final_progress)
        return 1
